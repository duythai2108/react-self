...
const Modal = ({ open = false, handleClose = () => {} }) => {
  ...
  return ReactDOM.createPortal(
    <div
      className={`modal flex items-center justify-center p-5 fixed inset-0 z-50 ${
    ...
    >
      ...
      <div className="modal-content bg-white relative z-10 p-10 rounded-lg w-full max-w-[482px]">
        <span
          onClick={handleClose}
          className="absolute top-0 right-0 cursor-pointer w-10 h-10 rounded-full bg-white p-1 flex items-center justify-center -translate-y-2/4 translate-x-2/4"
        >
          <svg
            width="14"
            height="14"
